...
func newFreeClientPool(db common.Database, connectedLimit, totalLimit int, clock mclock.Clock) *freeClientPool {
	pool := &freeClientPool{
		db:             db,
		clock:          clock,
...
func (f *freeClientPool) loadFromDb() {
	enc, err := f.db.GlobalTable().Get([]byte("freeClientPool"))
	if err != nil {
		return
	}
...
func (f *freeClientPool) saveToDb() {
	...
	if err != nil {
		...
		f.db.GlobalTable().Put([]byte("freeClientPool"), enc)
	}
}
